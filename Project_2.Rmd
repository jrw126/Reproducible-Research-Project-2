...
# Analysis
Which event types are the most harmful to population health? The variables `FATALITIES` and `INJURIES` should be helpful in answering this question.
```{r, echo=TRUE}
library(ggplot2)
library(plyr)
data$EVTYPE <- as.factor(data$EVTYPE)

deathRank <- aggregate(FATALITIES ~ EVTYPE, data = data, FUN = sum)     # Subset the data to only include the top 10 deadliest events
deathRank <- deathRank[order(-deathRank$FATALITIES), ]
deadliest <- as.character(deathRank$EVTYPE[1:10])
deadlyData <- data[data$EVTYPE %in% deadliest, ]
deadlyData.t <- ddply(deadlyData, .(EVTYPE), transform, len = length(FATALITIES))
ggplot(deadlyData.t, aes(x = BGN_DATE, color = EVTYPE)) + geom_step(aes(len = len, y = ..y.. * len), stat = "ecdf", size = 1)
#OR#
deadlyData.t <- ddply(deadlyData, .(EVTYPE), transform, cf = cumsum(FATALITIES))
ggplot(deadlyData.t, aes(x = BGN_DATE, y = cf, color = EVTYPE)) + geom_line()

# Cumulative amount of deaths per EVTYPE
ggplot(data, aes(x = BGN_DATE, y = cumsum(FATALITIES), col = EVTYPE)) + geom_line()

# Try this one with top 10 highest deaths only
data.t <- transform(data, y = runif(nrow(data)))
data.t <- ddply(data.t, .(EVTYPE), transform, cf = cumsum(FATALITIES))
qplot(x = BGN_DATE, y = cf, col = EVTYPE, data = data.t, geom = "smooth") + geom_line()


#a different attempt, THIS ONE WORKS
data.t2 <- ddply(data, .(EVTYPE), transform, len = length(FATALITIES))
ggplot(data.t2, aes(x = BGN_DATE, color = EVTYPE)) + geom_step(aes(len = len, y = ..y.. * len), stat = "ecdf", size = 1)
```
# Results
